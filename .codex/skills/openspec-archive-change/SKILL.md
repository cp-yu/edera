...
2.5. **Execute Full Verify**
   ...
   - In `expanded`, you MAY invoke `$openspec-verify-change` or execute the same contract inline, but the semantics MUST stay identical
...
5. **Assess delta sync state**
   ...
   **If any delta exists**:
   ...
   - In `expanded`, `$openspec-sync-specs` may still exist as a standalone workflow, but archive MUST follow the same sync-state contract
