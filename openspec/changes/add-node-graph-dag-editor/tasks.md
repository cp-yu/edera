...
- [x] A1 建立 Node Graph 编辑器 OpenSpec 文档，明确一个 change 内的多阶段范围、非目标、API 和验收边界。
- [x] A2 设计并实现 Graph ViewModel 后端层，提供节点原型、DAG 图状态、UI 布局元数据和运行状态映射，不让前端直接解析 YAML。
- [x] A3 引入无构建链节点图引擎静态资源，优先 LiteGraph.js，并新增 Node Graph 页面骨架：palette、canvas、Inspector、fallback 链接。
- [x] A4 实现 Phase 1 只读图展示：加载现有 DAG，显示节点、输入/输出端口、连线、fan flags 和类型标签。
- [x] A5 实现 Phase 2 图上编辑与保存：添加节点、拖拽定位、端口连线、删除节点/边、保存 DAG，并保持后端校验和原子写入。
- [x] A6 实现 Phase 3 Inspector 节点配置：展示并保存 `skills[]`、`model`、`source_names`、`timeout_seconds`、`parameters`。
- [x] A7 实现 Phase 4 运行状态叠加：将当前或最近 NodeRun 状态、错误和 cycle_id 映射到画布节点。
- [x] A8 调整配置页入口，使 Node Graph 成为 DAG 编辑主入口，保留表格/YAML 兜底入口。
- [x] A9 增加测试覆盖：Graph ViewModel round trip、DAG/Node 保存拒绝路径、页面加载、静态资源、运行状态映射和 fallback 可用性。
...
- [x] C2 验证 Graph ViewModel 与配置 round trip
  - Covers: A2, A5, A9
