...
主 DAG 运行时 SHALL 通过 `initial_payload: {"ticker": "<code>"}` 接收股票代码。preflight 节点从 payload 提取 ticker，并通过主 DAG 边传递给 `data_collection` Sub DAG。

#### Scenario: 手动触发传入 ticker

- **WHEN** 用户调用 `POST /api/dags/uzi-skill-analysis/run` 并传入 `{"ticker": "300470.SZ"}`
- **THEN** 主 DAG 的 initial_payload MUST 为 `{"ticker": "300470.SZ"}`

#### Scenario: ticker 传递到 Sub DAG

- **WHEN** `data_collection` Sub DAG 节点启动
- **THEN** 其 initial_payload MUST 包含 preflight 的输出（包含 ticker 信息）

### Requirement: UZI-Skill workflow extension imports

`extensions/uzi-skill/manifest.yaml` SHALL import UZI-Skill workflow-owned Entity files：主 DAG `uzi-skill-analysis`、3 个 Sub DAG Entity（`uzi-data-collection`, `uzi-scoring-synthesis`, `uzi-rendering`）、所有 `uzi-*` node 定义、`aggregate-collection-results` node 定义、`uzi-skill-analysis-default-cron` Trigger Entity 和 `v8_isolate` Resource Entity。Manifest MUST 保留 `legacy-script-adapter` handler 注册。

#### Scenario: Manifest imports 主 DAG 和 Sub DAG

- **WHEN** bootstrap 扫描 `extensions/uzi-skill/manifest.yaml`
- **THEN** manifest `imports.entities` SHALL 包含主 DAG Entity、3 个 Sub DAG Entity、所有 UZI node Entity 文件、aggregate node、UZI cron trigger Entity 和 `v8_isolate` Resource Entity
- **AND** HandlerRegistry MUST 仍包含 `legacy-script-adapter`

#### Scenario: Resource entity imported

- **WHEN** UZI-Skill imports 应用到空 DB
- **THEN** Entity Store SHALL 包含 Resource Entity `v8_isolate`
- **AND** 其 `permits` 属性 MUST 等于 1
