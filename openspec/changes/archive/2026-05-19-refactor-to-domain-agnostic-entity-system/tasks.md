...
- [x] A1 创建 entity types schema 定义（stock, city, rss-source, web-source）
- [x] A2 创建 entities.yaml 配置文件结构和加载逻辑
- [x] A3 创建 entity-relations.yaml 配置文件结构和加载逻辑
- [x] A4 实现实体引用解析（UUID 和业务 ID 双模式）
- [x] A5 实现字段权限系统（黑名单机制、枚举值、继承）
- [x] A6 实现节点实例权限提权验证（配置时检查）
- [x] A7 实现节点执行时权限检查（运行时检查）
- [x] A8 实现节点上下文实体访问接口（get_entity, save_entity, create_entity）
- [x] A9 实现实体自动发现逻辑（从 entity-relations.yaml）
- [x] A10 修改 RawItem 模型（stock_codes → tags）
- [x] A11 创建数据库迁移脚本（stock_codes → tags）
- [x] A12 新增实体配置 API 端点（GET/POST /api/config/entities）
- [x] A13 新增关系配置 API 端点（GET/POST /api/config/entity-relations）
- [x] A14 废弃 portfolio 配置 API 端点
- [x] A15 修改节点实例模型（新增 entities 和 entity_permissions 字段）
- [x] A16 修改 DAG YAML 序列化（包含 entities 和 entity_permissions）
- [x] A17 实现 Inspector 实体选择器组件
- [x] A18 实现 Inspector 权限配置器组件
- [x] A19 修改底部工具栏（target 过滤器 → entity 过滤器）
- [x] A20 创建 portfolio.yaml 到 entities.yaml 的迁移脚本

## 2. Checks

- [x] C1 验证 entity types schema 加载
  - Covers: A1
...
- [x] C3 验证 entity-relations.yaml 加载
  - Covers: A3
