...
- [x] A1 修改 `useRuntimeStatus` 接受 `polling` 参数，启用条件 `refetchInterval: polling ? 2000 : false`
- [x] A2 修改 `WorkbenchPage` 将 `isRunning` 传递给 `useRuntimeStatus`

## 2. Checks

- [x] C1 验证 `useRuntimeStatus` 支持条件轮询
  - Covers: A1
...
- [x] C2 验证 `WorkbenchPage` 正确传递轮询参数
  - Covers: A2
