...
- [ ] A1 创建 uv workspace 根 `pyproject.toml`，声明 `packages/core` 和 `packages/core-types` 为 workspace members
- [ ] A2 创建 `packages/core-types/` 包结构：`pyproject.toml` + `src/stockimformation_types/`，定义 `NodeInput`、`NodeOutput`、`HandlerContext`、`HandlerProtocol`、`EntityStoreProtocol`
- [ ] A3 创建 `packages/core/` 包结构：`pyproject.toml` + `src/stockimformation_core/`，依赖 `stockimformation-types`
- [ ] A4 迁移 `src/stockimformation/dag/` 到 `packages/core/src/stockimformation_core/dag/`（runner、loader、conditions、models）
- [ ] A5 迁移 `src/stockimformation/config/` 到 `packages/core/src/stockimformation_core/config/`（schema、loader、entities、git）
- [ ] A6 迁移 `src/stockimformation/trigger.py` 到 `packages/core/src/stockimformation_core/trigger.py`
- [ ] A7 迁移 `src/stockimformation/models/` 到 `packages/core/src/stockimformation_core/storage/`（database、repository），移除 `RawItem` 专用表定义，保留 `PipelineRun`、`NodeRun`、`NodeOutputEntity`
- [ ] A8 迁移 `src/stockimformation/errors.py` 到 `packages/core/src/stockimformation_core/errors.py`
- [ ] A9 实现 `packages/core/src/stockimformation_core/manifest.py` — manifest 解析、校验、NodeTypeDescriptor 提取、fallback 逻辑
- [ ] A10 实现 `packages/core/src/stockimformation_core/registry.py` — HandlerRegistry 和 EntityTypeRegistry
- [ ] A11 实现 `packages/core/src/stockimformation_core/bootstrap.py` — 扫描扩展目录、解析 manifest、构建 registry、设置 sys.path、声明式建表
- [ ] A12 重构 `packages/core/src/stockimformation_core/node/executor.py` — 移除 `_run_pi`/`_prepare_workspace`/LLM 分支，统一为 importlib 加载 + `HandlerContext` 单参数调用
- [ ] A13 重构 `packages/core/src/stockimformation_core/pipeline.py` — 移除 `_build_executor` 硬编码 handler 字典，改为从 bootstrap registry 获取
- [ ] A14 实现 `packages/core/src/stockimformation_core/engine.py` — 统一启动入口 `Engine(config_dir, extensions_dirs)`
- [ ] A15 创建 `extensions/_lib/http_fetch.py` — 从 `services/collection.py` 提取 HTTP 抓取、recovery、去重逻辑
- [ ] A16 创建 `extensions/_lib/llm.py` — 从 `node/executor.py` 提取 `_run_pi`、`_prepare_workspace` 逻辑
- [ ] A17 创建 `extensions/rss-fetcher/` — `manifest.yaml` + `handler.py`，实现 `async def run(ctx: HandlerContext)`
- [ ] A18 创建 `extensions/web-scraper/` — `manifest.yaml` + `handler.py`
- [ ] A19 创建 `extensions/api-fetcher/` — `manifest.yaml` + `handler.py`
- [ ] A20 创建 `extensions/reader/` — `manifest.yaml` + `handler.py`（内部调用 `_lib/llm.py`）
- [ ] A21 创建 `extensions/advisor/` — `manifest.yaml` + `handler.py`
- [ ] A22 创建 `extensions/briefing-generator/` — `manifest.yaml` + `handler.py`
- [ ] A23 创建 `extensions/notifier/` — `manifest.yaml` + `handler.py`
- [ ] A24 移动 `frontend/` 到 `apps/web-console/`，更新相关路径引用
- [ ] A25 更新所有测试文件的 import 路径，重组为 `tests/core/` 和 `tests/extensions/`
- [ ] A26 删除旧 `src/stockimformation/` 目录和旧 `handlers/`、`skill_handlers/`、`services/` 目录
- [ ] A27 更新根 `pyproject.toml` 入口点 `stockimformation = "stockimformation_core.main:main"`

## 2. Checks

- [ ] C1 验证 uv workspace 结构可解析
  - Covers: A1, A2, A3
...
- [ ] C5 验证 storage 模块迁移且无 RawItem 表
  - Covers: A7
...
- [ ] C8 验证 handler registry 构建和冲突检测
  - Covers: A10
...
- [ ] C11 验证 pipeline controller 无硬编码 handler
  - Covers: A13
