...
**Reason**: 该功能是文件系统时代的遗留机制，在数据库优先的架构中不再需要。所有扩展必须提供完整的 `manifest.yaml`。

**Migration**: 为所有使用 fallback 机制的扩展创建 `manifest.yaml` 文件，明确声明 handler 的 name、entry、role、input_type、output_type。
