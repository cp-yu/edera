...
- [ ] A1 在 `routes.py` 新增 `GET /api/config/portfolio` 端点，读取 `config/portfolio.yaml` 并返回 `{ "content": "<yaml text>" }`
- [ ] A2 在 `routes.py` 新增 `GET /api/config/system` 端点，读取 `config/system.toml` 并返回 `{ "content": "<toml text>" }`
- [ ] A3 在 `routes.py` 新增 `POST /api/graph/dag/{name}/nodes` 端点，创建节点 YAML 文件并追加到 DAG nodes 列表
- [ ] A4 为 `useRunDag`、`useStopDag`、`useCreateNode` mutation 添加 `onError` 回调，使用 `window.alert` 展示错误信息
- [ ] A5 在 `SourcesPage.tsx` 添加 error state 分支，API 失败时展示错误提示和重试按钮

## 2. Checks

- [ ] C1 验证 `GET /api/config/portfolio` 返回配置内容
  - Covers: A1
