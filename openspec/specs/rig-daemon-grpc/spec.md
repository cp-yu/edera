...
Daemon 启动 agent 节点前，SHALL 使用内部 CA 签发短期 client cert，CN 为 `node:{instance_id}`，TTL 对齐节点 timeout。
...
#### Scenario: 证书通过环境变量注入
- **WHEN** agent 节点启动
- **THEN** subprocess 环境变量 SHALL 包含 `RIG_CLIENT_CERT` 和 `RIG_CLIENT_KEY` 指向证书路径
