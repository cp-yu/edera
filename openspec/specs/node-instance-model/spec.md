...
节点实例 SHALL 仅能覆盖运行时参数，结构性字段由类型定义锁定。

#### Scenario: LLM instance configurable fields
- **WHEN** 用户编辑 LLM 节点实例配置
- **THEN** 系统 SHALL 允许修改：`alias`、`skills`（自由增删）、`model`、`parameters`

#### Scenario: Function instance configurable fields
- **WHEN** 用户编辑 Function 节点实例配置
- **THEN** 系统 SHALL 允许修改：`alias`、`source_names`、`parameters`
...
DAG YAML SHALL 使用实例对象列表格式存储节点。

#### Scenario: DAG YAML structure
- **WHEN** 系统保存 DAG 配置
- **THEN** 系统 SHALL 将每个节点序列化为包含 `id`（UUID）、`type`（类型名引用）、`alias`（可选）、`config`（实例级覆盖）的对象
