...
- **WHEN** 反思 DAG 的 source 节点通过 `rig entity query` 查询目标节点的历史输出
...
#### Scenario: 手动触发
- **WHEN** 用户通过 `rig dag trigger reflection-dag` 手动触发
...
#### Scenario: 查询反思关系
- **WHEN** 用户执行 `rig entity query "relation_type=reflects AND to=default:llm-analyze"`
