...
from edera_core.config.entities import EntityStore, can_write, field_permission
...
from edera_core.grpc_runtime import load_rig_proto
from edera_core.pipeline import PipelineController
from edera_core.rig_cli import _entity_payload, _query
...
def resolve_data_dir(cli_arg: Path | str | None = None) -> Path:
    ...
    default = Path("/var/lib/edera")
    if _writable_dir(default):
        return default
    return Path.home() / ".local" / "share" / "edera"
