...
@pytest.mark.asyncio
async def test_delete_entity_type(tmp_path):
    root = tmp_path / "config"
    _write_config(root)
    service = _ConfigService(FakeDaemon(root))

    result = await service.DeleteEntityType(pb2.DeleteEntityTypeRequest(name="stock", cascade=False), FakeContext())

    assert "stock.yaml" not in {path.name for path in (root.parent / "schemas" / "entity-types").glob("*.yaml")}
    assert '"deleted": true' in result.json
