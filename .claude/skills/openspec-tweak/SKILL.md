...
6. For each artifact (proposal, specs), run `openspec instructions <artifact-id> --change "<name>" --json`; read `configProjection` (especially `configProjection.normalized.proseLanguage` and `configProjection.prompt.fragments`), dependencies, `template`, `instruction`, and `outputPath`; follow the template exactly and do not copy `context`, `rules`, or `configProjection` into artifact files. When creating `specs`, apply the returned `Spec content boundary`:
   - Allowed: externally observable behavior, CLI/API inputs/outputs, errors, validation, permissions, persistence, compatibility, testable constraints
   - Forbidden: implementation path, refactor rationale, rejected approaches, exploration notes, code organization, library choices, agent reasoning
   - Route forbidden content: technical decisions→design.md, implementation steps→tasks.md, motivation→proposal.md, architecture changes→opsx-delta.yaml
...
9. Finish with `openspec status --change "<name>"` and explain the two usage paths:
   - **Doc-first**: Use generated specs to guide code implementation, then run `/opsx:archive`
   - **Code-first**: Code is already written, specs capture what changed, then run `/opsx:archive`
