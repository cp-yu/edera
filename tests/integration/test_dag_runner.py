...
from stockimformation.dag.loader import load_graph, topological_layers, validate_sub_dag_nesting
...
@pytest.mark.asyncio
async def test_fallback_switch_model() -> None:
    config = load_app_config(Path("config"))
    nodes = _condition_nodes()
    graph = load_graph(
        config.dags["default"].model_validate(
            {
                "name": "fallback-test",
                "nodes": [
                    {"id": "llm", "type": "advisor", "fallback": "switch_model", "fallback_model": "backup"}
                ],
                "edges": [],
            }
        ),
    ...
    async def handler(_payload: object, parameters: dict[str, object], _context: object) -> object:
        if parameters.get("model") == "backup":
            return {"model": parameters["model"]}
        raise RuntimeError("primary failed")

    executor = NodeExecutor(nodes, config.system, config.runtime, {"generate-advice": handler}, graph.instances)
    result = await DagRunner(executor).run(graph, "cycle", {})

    assert result.node_outputs["llm"].payload == {"model": "backup"}
