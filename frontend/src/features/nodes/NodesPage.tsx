...
type Tab = 'llm' | 'function' | 'skills' | 'handlers'
...
export function NodesPage() {
  ...
  const nodes = tab === 'llm' || tab === 'function'
    ...
    <main className="h-full overflow-y-auto bg-background p-6">
      <div className="mx-auto max-w-6xl space-y-5">
        ...
        <div className="flex gap-2">
          {(['llm', 'function', 'skills', 'handlers'] as Tab[]).map((item) => (
            <button
              key={item}
              onClick={() => setTab(item)}
              className={`rounded-md border px-3 py-1.5 text-sm ${tab === item ? 'bg-primary text-primary-foreground' : 'bg-card'}`}
            >
              {item === 'llm' ? 'LLM 节点' : item === 'function' ? 'Function 节点' : item === 'skills' ? 'Skills' : 'Handler'}
            </button>
          ))}
        </div>
