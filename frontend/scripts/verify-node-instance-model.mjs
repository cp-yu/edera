...
const fixtures = {
  nodeTypes: [
    ...
    nodeType('reader', 'llm', 'processor', 'list[RawItem]', 'AnalysisResult', {
      skills: ['summarize'],
      model: 'hf-share/deepseek-v4-flash',
      system_prompt_file: 'prompts/reader.md',
    }),
    nodeType('advisor', 'llm', 'processor', 'AnalysisResult', 'Advice', {
      model: 'hf-share/deepseek-v4-flash',
  ...
  const checks = await evaluate(cdp, `
    (async () => {
      ...
      return {
        ...
        warnsLlmMismatch: graph.isValidConnection(source, llmTarget) === true &&
          graph.isWarningConnection(source, llmTarget) === true,
...
async function verifyQuickAddAndMultiInstance(cdp) {
  ...
  const aliasSearch = await evaluate(cdp, `
    (async () => {
      ...
      return buttons.includes('readerllm') || buttons.includes('reader')
    })()
  `)
...
async function verifyNodesPage(cdp, baseUrl) {
  ...
  await waitFor(cdp, `document.body.textContent.includes('LLM')`)
  ...
  const checks = await evaluate(cdp, `
    (async () => {
      ...
      const hasLlm = document.body.textContent.includes('reader')
      await clickTab('Function')
      ...
      return {
        ...
        hasCodeEditor: editorText.includes('handler_code') || editorText.includes('system_prompt'),
      }
    })()
  `)
