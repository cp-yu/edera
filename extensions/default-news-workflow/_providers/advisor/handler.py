...
async def run(ctx: HandlerContext) -> list[dict[str, Any]]:
    ...
    return [
        _generate_advice(target, analyses).model_dump(mode="json")
        for target in ctx.entity_store.query("stock")
    ]


def _generate_advice(target: EntityConfig, analyses: list[AnalysisResult]) -> Advice:
