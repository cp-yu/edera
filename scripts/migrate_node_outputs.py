...
def _ensure_table(conn: sqlite3.Connection) -> None:
    conn.execute(
        ...
        "run_id TEXT NOT NULL, node_id TEXT NOT NULL, payload TEXT NOT NULL, "
...
def _copy_rows(conn: sqlite3.Connection, table: str, entity_type: str, node_id: str) -> None:
    ...
    for row in conn.execute(f"SELECT * FROM {table}"):
        ...
        conn.execute(
            ...
            "(entity_id, type, run_id, node_id, payload, tags, session_id, url, created_at) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (uuid4().hex, entity_type, run_id, node_id, json.dumps(payload), json.dumps(tags), None, url, created_at),
        )
